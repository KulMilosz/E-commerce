...
async function main() {
  ...
  const monitorProducts = [
    ...
    },
    {
      ...
      description: 'Gaming monitor with 144Hz refresh rate and 1ms response time',
      price: 249.99,
      stock: 20,
      imageUrl: 'https://via.placeholder.com/300x300'
    },
  ...
  const headphoneProducts = [
    ...
    },
    {
      ...
      description: 'Professional studio monitor headphones with detachable cable',
      price: 149.99,
      stock: 35,
      imageUrl: 'https://via.placeholder.com/300x300'
    },
  ...
  const keyboardProducts = [
    ...
    },
    {
      ...
      description: 'Wireless keyboard with backlit keys and multi-device connectivity',
      price: 99.99,
      stock: 25,
      imageUrl: 'https://via.placeholder.com/300x300'
    },
  ...
  await prisma.orderItem.create({
    data: {
      orderId: order1.id,
      productId: (await prisma.product.findFirst({ where: { name: 'Logitech MX Master 3S' } }))!.id,
      quantity: 1,
  ...
  await prisma.orderItem.create({
    data: {
      orderId: order1.id,
      productId: (await prisma.product.findFirst({ where: { name: 'Sony WH-1000XM5' } }))!.id,
      quantity: 1,
  ...
  await prisma.orderItem.create({
    data: {
      orderId: order2.id,
      productId: (await prisma.product.findFirst({ where: { name: 'Dell UltraSharp 27" 4K' } }))!.id,
      quantity: 1,
      priceAtPurchase: 399.99
    }
  })
